...
| Compression competitiveness | Multi-candidate managed Deflate, native zlib/libdeflate, optional native Zopfli, configurable block size, and threaded pipeline are implemented. Independent Hakamiq benchmark coverage must be rerun. | PENDING BENCHMARK |
...
- Rerun the real ISO benchmark using Hakamiq profiles only after committing this phase.
